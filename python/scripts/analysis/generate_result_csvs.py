...
DEFAULT_ALGORITHMS = [
    ...
    ("BRKGA", "results/experiments/brkga"),
]
...
METAHEURISTICS = ["ACO_TS", "HHO_RVNS", "BRKGA"]
METHOD_COLUMNS = {"PLI": "Objetivo", **{method: method for method in METAHEURISTICS}}
STOP_REASON_LABELS = {
                    ...
                    convergence_rows.append(
                        {
                            ...
                            "iteration": point.get(
                                "iteration", point.get("generation")
                            ),
...
def plot_stop_reason_counts(
    ...
    for algorithm in algorithms:
        ...
        if column not in df.columns:
            continue
        reasons = df[column].fillna("indefinido").map(
    ...
    width = min(0.8 / len(methods), 0.38)
...
def method_value_table(df: pd.DataFrame, methods: list[str]) -> pd.DataFrame:
    ...
    for method in methods:
        table[method] = numeric_series(df, METHOD_COLUMNS[method])
...
def run_metaheuristic_wilcoxon(
    ...
    for dataset, subset in groups:
        objective = numeric_series(subset, "Objetivo").replace(0, math.nan)
        gap_sets = {
            "gap_relativo_melhor": {
                method: numeric_series(subset, f"{method}_gap_relativo")
                for method in METAHEURISTICS
            },
            "gap_relativo_mediano_das_10_execucoes": {
                method: 100.0
                * (numeric_series(subset, f"{method}_mediana") - objective)
                / objective
                for method in METAHEURISTICS
            },
        }
        for variable, gaps in gap_sets.items():
            for method_a, method_b in combinations(METAHEURISTICS, 2):
                paired = pd.DataFrame(
                    {"a": gaps[method_a], "b": gaps[method_b]}
                ).dropna()
                ...
                rows.append(
                    {
                        "dataset": dataset,
                        "comparacao": f"{method_a}_vs_{method_b}",
                        "metodo_a": method_a,
                        "metodo_b": method_b,
                        "variavel": variable,
                        "n_instancias": len(paired),
                        "a_menor_gap": int((differences < -EPS).sum()),
                        "b_menor_gap": int((differences > EPS).sum()),
                        "empates": int((differences.abs() <= EPS).sum()),
                        "mediana_gap_a": float(paired["a"].median()),
                        "mediana_gap_b": float(paired["b"].median()),
                        "mediana_diferenca_a_menos_b": float(
                            differences.median()
                        ),
                        "estatistica": statistic,
                        "p_valor": p_value,
...
def plot_metaheuristic_gap_scatter(combined: pd.DataFrame, output_dir: Path) -> int:
    ...
    for method_a, method_b in combinations(METAHEURISTICS, 2):
        paired = pd.DataFrame(
            {
                "x": numeric_series(combined, f"{method_a}_gap_relativo"),
                "y": numeric_series(combined, f"{method_b}_gap_relativo"),
        ...
        low = min(float(paired["x"].min()), float(paired["y"].min()))
        high = max(float(paired["x"].max()), float(paired["y"].max()))
        ax.plot([low, high], [low, high], color="#333333", linewidth=1.0)
        ax.set_title("Comparação do Gap Relativo")
        ax.set_xlabel(f"Gap relativo de {plain_method_label(method_a)} (%)")
        ax.set_ylabel(f"Gap relativo de {plain_method_label(method_b)} (%)")
        place_legend_inside_upper_right(ax, columns=3)
        filename = (
            "dispersao_gap_relativo_metaheuristicas.png"
            if (method_a, method_b) == ("ACO_TS", "HHO_RVNS")
            else "dispersao_gap_relativo_"
            f"{method_a.lower()}_{method_b.lower()}.png"
        )
        finish_summary_plot(fig, ax, output_dir / filename)
        count += 1
    return count
...
def gap_summary_by_group(combined: pd.DataFrame) -> pd.DataFrame:
    ...
    for dataset, subset in groups:
        row: dict[str, Any] = {"dataset": dataset, "n_instancias": len(subset)}
        medians: dict[str, float] = {}
        for method in METAHEURISTICS:
            gap = numeric_series(subset, f"{method}_gap_relativo")
            row[f"{method}_gap_medio"] = float(gap.mean())
            row[f"{method}_gap_mediano"] = float(gap.median())
            medians[method] = float(gap.median())
        best = min(medians.values())
        winners = [
            method
            for method, median in medians.items()
            if math.isclose(median, best, abs_tol=EPS)
        ]
        row["melhor_mediana"] = ",".join(winners) if len(winners) > 1 else winners[0]
        rows.append(row)
    return pd.DataFrame(rows)
...
def run_statistical_analysis(
    ...
    run_friedman_with_pli(combined, stats_dir)
    run_pairwise_wilcoxon_with_pli(combined, stats_dir)
    run_metaheuristic_wilcoxon(combined, stats_dir)
    gap_summary_by_group(combined).to_csv(
        stats_dir / "resumo_gaps_metaheuristicas.csv", index=False
    )
    return 0
...
def build_general_summary(combined: pd.DataFrame) -> pd.DataFrame:
    ...
    for dataset, subset in groups:
        ...
        for method in METAHEURISTICS:
            row[f"{method}_melhor_ou_empate"] = winners[method]
            row[f"{method}_vitoria_estrita"] = strict_winners[method]

        for prefix in METAHEURISTICS:
    ...
    for dataset, subset in groups:
        ...
        for method in methods:
            ...
            row = {
                ...
                **{
                    f"igual_ao_{other}": (
                        int((values[method] == values[other]).sum())
                        if method != other
                        else len(subset)
                    )
...
def median_run_robustness_summary(combined: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    groups: list[tuple[str, pd.DataFrame]] = [("TODOS", combined)]
    groups.extend(
        (dataset, subset) for dataset, subset in combined.groupby("dataset", sort=True)
    ...
    for dataset, subset in groups:
        ...
        best = paired.min(axis=1)
        best_mask = paired.sub(best, axis=0).abs().le(EPS)
        row: dict[str, Any] = {
            "dataset": dataset,
            "n_instancias": len(paired),
            "empates_gap_mediano": int(best_mask.sum(axis=1).gt(1).sum()),
        }
        for method in METAHEURISTICS:
            row[f"{method}_gap_mediano_10_execucoes"] = float(
                paired[method].median()
            )
            row[f"{method}_melhor_gap_mediano"] = int(best_mask[method].sum())
        rows.append(row)

    return pd.DataFrame(rows)


def write_summary_csvs(combined: pd.DataFrame, output_dir: Path) -> Path:
    ...
    robustness_summary = median_run_robustness_summary(combined)
    ...
    robustness_summary.to_csv(
        output_dir / "resumo_robustez_metaheuristicas.csv", index=False
    )
    return output_dir / "resumo_resultados.csv"
...
def main() -> int:
    ...
    try:
        ...
        run_statistical_analysis(combined, output_dir)
        print(
            "CSVs estatisticos salvos em: "
        ...
        summary_path = write_summary_csvs(combined, output_dir)
        print(f"CSVs de resumo salvos em: {summary_path.parent.relative_to(PROJECT_ROOT)}")
